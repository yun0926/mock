...
function dragula (initialContainers, options) {
  ...
  if (o.drag === void 0) { o.drag = function(){} }
  var drake = emitter({
  ...
  function release (e) {
    ...
    if (dropTarget && ((_copy && o.copySortSource) || (!_copy || dropTarget !== _source))) {
      ...
      cancel(null,elementBehindCursor,_item);
    }
  ...
  function cancel (revert,elementBehindCursor,sourceTarget) {
    if (!drake.dragging) {
      return;
    }
    ...
    cleanup(elementBehindCursor,sourceTarget);
  }

  function cleanup (targetEl,sourceTarget) {
    var item = _copy || _item;
    ungrab();
    removeMirrorImage();
    if (item) {
    ...
    if (_lastDropTarget) {
      drake.emit('out', targetEl,sourceTarget,item, _lastDropTarget, _source);
    }
  ...
  function drag (e) {
    ...
    var elementBehindCursor = getElementBehindPoint(_mirror, clientX, clientY);
    o.drag && o.drag(e,_mirror,x,y,elementBehindCursor);
    var item = _copy || _item;
    var dropTarget = findDropTarget(elementBehindCursor, clientX, clientY);
