...
!function($) {
   ...
   $.fn.validation.defaults = {
        validRules : [
            ...
            {name: 'checkpd', validate: function(value) {return !($("input[type=password]:first").val() === value);}, defaultMsg: '两次输入的密码不一致' },
            {name: 'checkun', validate: function(value) {return !(/^[a-z]\w{4,50}$/.test(value));},defaultMsg:'用户名至少包含5-50个字符,且以字母开头。'},
    ...
    var validateField = function(field, valid) { 
        ...
        if(form){
            ...
            if(fstyle == 0){
                ...
                el.after('<span class="validate-error help-block" id="valierr">' + errorMsg +'</span>');
            }
            else if(fstyle == 1){
                ...
                controlGroup.find("#valierr").remove();
                el.parent().after('<span class="validate-error help-block" id="valierr">' + errorMsg +'</span>');
            }
        ...
        $(obj).find('input, textarea').each(function(){
            ...
            el.on('blur',function(){ // 失去焦点时
                ...
                if (valid){
                    ...
                    //add sky
                    var arrErr = $(obj).find('[id=valierr]');
